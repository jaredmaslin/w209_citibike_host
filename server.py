...
def get_trips_data(es, outfile, dates=None, usertype=None, gender=None, min_age=0, max_age=99, min_hour=0, max_hour=23, incoming=1):
...
def get_sid_trips_data(es, outfile, station_id=None, dates=None, usertype=None, gender=None, min_age=0, 
    max_age=99, min_hour=0, max_hour=23, incoming=1):
    query = '{ "query": { "filtered": { "query":{"match_all":{}}, "filter": { "bool": {'
    conditions=[]
    ...
    if incoming == 1:
        # incoming station is the user specified station, 
        # aggregated over start station id to get count of trips from different start station id
        conditions.append('"must": {"term": {"endID":' + str(station_id)+'} }')
        query = query + ",".join(conditions) + '}}}},"aggregations":{"startID":{"terms":{"field":"startID","size":0}}}}'
        json_ID_field='startID'
    else:
        # outgoing station is the user specified station, 
        # aggregated over end station id to get count of trips from different end station id
        conditions.append('"must": {"term": {"startID":' + str(station_id)+'} }')
        query = query + ",".join(conditions) + '}}}},"aggregations":{"endID":{"terms":{"field":"endID","size":0}}}}'
        json_ID_field='endID'
...
def aggregate_trip_by_date(es, outfile, stationId=None, dates=None, usertype=None, gender=None, min_age=0, 
    max_age=99, min_hour=0, max_hour=23, incoming=1):
    query = '{ "query": { "filtered": { "query":{"match_all":{}}, "filter": { "bool": {'
    conditions=[]
...
def query_es():
    ...
    tmp_out_file = 'static/tmp_data/query_es_output'+time_tag+'.csv'
    #get user input
    ...
    get_trips_data(es, dates=dates, usertype=usertype, gender=gender, min_age=min_age, 
                    max_age=max_age, min_hour=min_hour, max_hour=max_hour, incoming=incoming, outfile=tmp_out_file)

    # dump data in csv file into json object
    fopen = open(tmp_out_file,'r')
    out = fopen.read()
    # Open the CSV  
...
@app.route('/query_es_sid', methods=['GET'])
def query_es_sid():
    time_tag = str(time.time())
    tmp_out_file = 'static/tmp_data/query_es_sid_output_'+time_tag+'.csv'
    #get user input
    ...
    get_sid_trips_data(es, station_id=station_id, dates=dates, usertype=usertype, gender=gender, min_age=min_age, 
                    max_age=max_age, min_hour=min_hour, max_hour=max_hour, incoming=incoming, outfile=tmp_out_file)
...
def query_daily_trip_count():
    time_tag = str(time.time())
    tmp_out_file = 'static/tmp_data/query_daily_trip_count_output_'+time_tag+'.csv'
    #get user input
    station_id=request.args.get('station_id')
    heavy_rain=request.args.get('heavy_rain') # 1 or 0
    heavy_snow=request.args.get('heavy_snow') # 1 or 0
